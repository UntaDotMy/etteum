...
interface AlibabaQuotaTokens {
  modelQuotas: Record<string, AlibabaModelQuota>;
  updatedAt: string;
}
...
export default function Accounts() {
  ...
  const [byokBulkKeysText, setByokBulkKeysText] = useState("");
  const [byokBulkKeysOpen, setByokBulkKeysOpen] = useState(false);
  ...
  /** Per-provider interval: explicit override → global default → 15 */
  function warmupIntervalFor(provider: Provider): number {
    const perKey = `auto_warmup_interval_${provider}`;
    const perVal = Number(settingsMap[perKey]);
    if (Number.isFinite(perVal) && perVal > 0) return Math.floor(perVal);
    return autoWarmup?.intervalMinutes ?? 15;
  }

  /** Per-provider next-run countdown label */
  function warmupCountdownFor(provider: Provider): string {
    const nextStr = autoWarmup?.providerNextRunAt?.[provider];
    if (!nextStr) return "—";
    const remaining = Math.max(0, new Date(nextStr).getTime() - now);
  ...
  async function handleWarmupIntervalChange(provider: Provider, value: string) {
    const num = Number(value);
    const key = `auto_warmup_interval_${provider}`;
    const prev = settingsMap[key];
    setSettingsMap((current) => ({ ...current, [key]: value }));
    try {
      // If empty or invalid, delete the per-provider override (fall back to global)
      if (!value || !Number.isFinite(num) || num <= 0) {
        await updateSettings({ [key]: "" });
      } else {
        const clamped = Math.min(1440, Math.max(1, Math.floor(num)));
        await updateSettings({ [key]: String(clamped) });
      }
      const status = await fetchAutoWarmupStatus();
      setAutoWarmup(status);
    } catch (err) {
      setSettingsMap((current) => ({ ...current, [key]: prev }));
      showError(err);
    }
  }
  ...
  async function handleCodeBuddyChinaBulkApiKey() {
    ...
    try {
      ...
      const cbMsg = res.skipped > 0
        ...
        : `Added ${res.count} CodeBuddy CN account(s) successfully`;
      showSuccess(cbMsg);
  ...
  async function handleAlibabaBulkApiKey() {
    const keysText = alibabaBulkApiKeys.trim();
    if (!keysText) { showError(new Error("Paste Alibaba DashScope API keys")); return; }
    ...
    try {
      ...
      const msg = res.skipped > 0
        ...
        : `Added ${res.count} Alibaba API key(s) successfully`;
      showSuccess(msg);
      setAlibabaBulkApiKeys("");
      setAddDialogProvider(null);
      await load();
    } catch (err) { showError(err); }
    finally { setAlibabaBusy(false); }
  }
  ...
  async function handleRetryErrors(provider: Provider) {
    ...
    if (NON_LOGINABLE.has(provider)) {
      try {
        ...
        showSuccess(`Queued ${ids.length} ${labelProvider(provider)} error accounts for WarmUp.`);
      } catch (err) { showError(err); }
      return;
    }
  ...
  function handleBulkImportKeys() {
    ...
    for (const line of lines) {
      ...
      if (!trimmed) continue;
      // Support "label:key" or "label key" or "key" alone
      let label = "";
      let key = "";
  ...
  async function handleFetchModels() {
    if (!byokForm.base_url) {
      showError(new Error("Enter a Base URL first"));
      return;
    }
    const firstKeyWithValue = byokForm.keys.find((k) => k.key && k.key !== BYOK_KEY_PLACEHOLDER);
    const apiKey = firstKeyWithValue?.key || "";
    if (!apiKey) {
      showError(new Error("Enter at least one API key to fetch models"));
      return;
    }
    ...
    try {
      ...
      if (result.models && result.models.length > 0) {
        setByokForm((f) => ({ ...f, models: result.models!.join(", ") }));
        showSuccess(`Found ${result.models!.length} models`);
      } else if (result.warning) {
        showError(new Error(result.warning));
      } else {
        showError(new Error("No models returned from upstream"));
      }
    } catch (err) {
      showError(err);
    } finally {
      ...
      if (provider === "alibaba") {
        ...
        for (const a of activeRows) {
          ...
          for (const key of KEY_MODELS) {
            ...
            modelQuotasSum[key]!.limit += q.limit;
            modelQuotasSum[key]!.remaining += q.remaining;
            modelQuotasSum[key]!.count++;
          }
        }
        // Remove models that have no data across any account
        for (const key of Object.keys(modelQuotasSum)) {
          if (modelQuotasSum[key]!.limit === 0) delete modelQuotasSum[key];
        }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {providerStats.map((stat) => (
          <Card
            key={stat.provider}
            className="border-[var(--border)] cursor-pointer hover:border-[var(--primary)]/50 transition-colors flex flex-col"
          ...
          >
            ...
            <CardContent className="space-y-4 flex flex-col grow">
              ...
              {/* Credits remaining — hide when no accounts */}
              {/* Credits remaining — hide when no accounts */}
              {stat.provider === "alibaba" && stat.modelQuotas ? (
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-[var(--muted-foreground)]">Model Quotas</span>
                  </div>
                  {Object.entries(stat.modelQuotas).map(([model, q]) => {
                    ...
                    return (
                      <div key={model} className="space-y-0">
                        <div className="flex items-center justify-between text-[10px]">
                          ...
                          <span className={`text-[var(--muted-foreground)] shrink-0 ml-2 ${exhausted ? "text-[var(--error)]" : ""}`}>
                            {q.remaining.toLocaleString()}/{q.limit.toLocaleString()} ({q.count} acc)
                          </span>
                        </div>
              ...
              {/* Bottom-docked: Auto WarmUp + Buttons */}
              <div className="mt-auto space-y-4">
              {/* Auto WarmUp toggle + per-provider interval */}
              <div
                className="flex flex-col gap-2 rounded-md border border-[var(--border)] bg-[var(--secondary)]/40 p-2"
              ...
              >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[var(--muted-foreground)] leading-tight">
                        {autoWarmupEnabledFor(stat.provider)
                          ? `Next in ${warmupCountdownFor(stat.provider)}`
                          : "Disabled"}
                      </p>
                    </div>
                  </div>
                ...
                {autoWarmupEnabledFor(stat.provider) && (
                  <div className="flex items-center gap-2 pl-6">
                    <label className="text-[10px] text-[var(--muted-foreground)] whitespace-nowrap">Every</label>
                    <input
                      type="number"
                      min={1}
                      max={1440}
                      value={warmupIntervalFor(stat.provider)}
                      onChange={(e) => handleWarmupIntervalChange(stat.provider, e.target.value)}
                      className="h-6 w-14 rounded border border-[var(--border)] bg-[var(--background)] px-1.5 text-xs text-[var(--foreground)]"
                      title={`WarmUp interval in minutes (default: ${autoWarmup?.intervalMinutes ?? 15}m)`}
                    />
                    <span className="text-[10px] text-[var(--muted-foreground)]">min</span>
                    {warmupIntervalFor(stat.provider) === (autoWarmup?.intervalMinutes ?? 15) && (
                      <span className="text-[9px] text-[var(--muted-foreground)]/60">(default)</span>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-2" onClick={(e) => e.stopPropagation()}>
                ...
                <Button className="w-full" variant="outline" size="sm" onClick={() => handleRetryErrors(stat.provider)} disabled={stat.error === 0}>
                  <RotateCcw className="mr-1 h-4 w-4" /> {NON_LOGINABLE.has(stat.provider) ? "Recheck" : "Retry"}
                </Button>
              </div>
              </div>
            </CardContent>
          ...
          <div className="space-y-4 pt-3">
            ...
            <div className="space-y-3 rounded-lg border border-[var(--border)] bg-[var(--secondary)]/[0.06] p-3.5">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <Button type="button" variant="outline" size="sm" className="h-7 gap-1 text-xs" onClick={() => setByokBulkKeysOpen(!byokBulkKeysOpen)}>
                    <FileText className="h-3 w-3" /> Bulk
                  </Button>
              ...
              {byokBulkKeysOpen && (
                <div className="rounded-md border border-[var(--border)] bg-[var(--card)] p-2.5 space-y-2">
                  <textarea
                    value={byokBulkKeysText}
                    onChange={(e) => setByokBulkKeysText(e.target.value)}
                    placeholder={'mykey:sk-...\nbackup:sk-...\npro:sk-...'}
                    className="w-full h-24 rounded-md border border-[var(--border)] bg-[var(--background)] p-2 text-xs font-mono text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-1 focus:ring-[var(--ring)] resize-none"
                  />
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] text-[var(--muted-foreground)]">Paste one key per line. Format: <code className="bg-[var(--secondary)] px-1 rounded">label:key</code> or <code className="bg-[var(--secondary)] px-1 rounded">label key</code> or just <code className="bg-[var(--secondary)] px-1 rounded">key</code></p>
                    <div className="flex gap-1.5">
                      <Button type="button" variant="ghost" size="sm" className="h-7 text-xs" onClick={() => { setByokBulkKeysText(""); setByokBulkKeysOpen(false); }}>Cancel</Button>
                      <Button type="button" variant="outline" size="sm" className="h-7 text-xs" onClick={handleBulkImportKeys}>Import</Button>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-3 rounded-lg border border-[var(--border)] bg-[var(--secondary)]/[0.06] p-3.5">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    {byokFetchingModels ? <Loader2 className="h-3 w-3 animate-spin" /> : <Search className="h-3 w-3" />}
                    Fetch Models
                  </Button>
                </div>
                <textarea
                ...
                />
                <p className="text-xs text-[var(--muted-foreground)]">Comma-separated list of model IDs. Click Fetch to auto-discover from the upstream API.</p>
              </div>
            </div>
...
ck_xyz789ghi012..."
          ...
          {addMode === "apikey" && addDialogProvider === "alibaba" && (
            <div className="space-y-4">
              <div>
                <label className="text-sm text-[var(--foreground)]">API Keys (satu per baris)</label>
                <textarea
                  value={alibabaBulkApiKeys}
                  onChange={(e) => setAlibabaBulkApiKeys(e.target.value)}
                  className="mt-1 w-full h-40 rounded-md border border-[var(--border)] bg-[var(--background)] p-3 text-sm font-mono text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-1 focus:ring-[var(--ring)] resize-none"
                  placeholder="sk-ws-H.xxxxxxxx...
sk-ws-H.yyyyyyyy...
sk-ws-H.zzzzzzzz..."
                  disabled={alibabaBusy}
                />
                <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                  Paste satu atau lebih Alibaba DashScope API key (prefix <code>sk-ws-</code>), satu per baris.
                  Endpoint: <code>https://dashscope-intl.aliyuncs.com/compatible-mode/v1</code>.
                  Model tersedia: <code>ali-qwen-plus</code>, <code>ali-qwen3.5-flash</code>, <code>ali-deepseek-v4-flash</code>, dll.
                </p>
              </div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setAddDialogProvider(null)} disabled={alibabaBusy}>Cancel</Button>
                <Button onClick={handleAlibabaBulkApiKey} disabled={alibabaBusy || !alibabaBulkApiKeys.trim()}>
                  {alibabaBusy ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Importing...</>) : "Add Accounts"}
                </Button>
              </div>
            </div>
          )}
