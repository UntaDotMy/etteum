...
export class CodexProvider extends BaseProvider {
   ...
   * immediately. Mirrors reference consumeCodexRateLimitResetCredit. Returns the
   * remaining credit count (0 = no credits left / consumed). Server-side
   * `redeemRequestId` (anti-replay) is generated by the caller (the API route).
   */
  async consumeResetCredit(
    ...
    try {
      const response = await this.fetchWithTimeout(CODEX_USAGE_URL, {
        ...
        body: JSON.stringify({ action: "redeem_rate_limit_reset", redeem_request_id: redeemRequestId }),
      }, config.providerQuotaTimeoutMs, signal);
      ...
      if (response.status === 409) {
        return { success: false, error: "no_credit", remainingCredits: 0 };
      }
      if (!response.ok) {
        const text = await response.text().catch(() => "");
        return { success: false, error: `HTTP ${response.status}: ${text.slice(0, 200)}` };
      ...
      const data = await response.json() as any;
      const remaining = data?.rate_limit_reset_credits?.available_count ?? data?.availableCount ?? 0;
