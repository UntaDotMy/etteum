...
describe("getProviderForModel — strict per-provider routing (no kiro catch-all)", () => {
  ...
  test("providers.grok is the first-party GrokProvider; no parallel xai catalog", () => {
    ...
    expect(getProviderForModel("grok-composer-2.5-fast")).toBe("grok");
    expect(getProviderForModel("composer-2.5-fast")).toBe("grok");
    // Common typo alias (groq vs grok) still routes to Grok provider.
    expect(getProviderForModel("groq-composer-2.5-fast")).toBe("grok");
    ...
    expect(getProviderForModel("grok-imagine-video-1.5")).toBeNull();
