 ...
 *  Excludes key-based providers (byok, codebuddy-china, youmind) that
 *  authenticate via API key / PAT paste, not browser automation. */
...
authRouter.post("/bulk-add", async (c) => {
  ...
  const providers = body.providers?.length ? body.providers : LOGINABLE_PROVIDERS;

  // Validate providers against the loginable set (excludes byok/youmind
  // which use API keys, not browser login)
  const validProviders = providers.filter((p) => LOGINABLE_PROVIDERS.includes(p));

  if (validProviders.length === 0) {
    return c.json({ error: `At least one valid provider is required (${LOGINABLE_PROVIDERS.join(", ")})` }, 400);
  }
...
authRouter.post("/import", async (c) => {
  ...
  const providers = (body.providers?.length ? body.providers : LOGINABLE_PROVIDERS).filter((p) =>
    LOGINABLE_PROVIDERS.includes(p)
  );
...
authRouter.post("/warmup-all", async (c) => {
  ...
  return c.json({ message: `Queued ${count} accounts for WarmUp`, count, concurrency: warmupQueue.getStatus().concurrency });
});
