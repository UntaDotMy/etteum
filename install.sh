...
show_summary() {
  ...
  for cand in python3.12 python3.11 python3.10 python3 python3.13; do
    if have "$cand"; then
      ...
      _v=$("$cand" -c 'import sys;print("%d.%d"%sys.version_info[:2])' 2>/dev/null || echo "0.0")
      IFS=. read -r major minor <<<"$_v"
...
ensure_bun() {
  local BUN_BAD_VERSION="1.3.14"
  if have bun; then
    local bun_ver
    bun_ver=$(bun --version 2>/dev/null || echo "")
    if [[ "$bun_ver" == "$BUN_BAD_VERSION" ]]; then
      step "Bun $bun_ver detected — upgrading to canary (stable $BUN_BAD_VERSION has known issues)"
      if ! bun upgrade --canary; then
        err "Bun canary upgrade failed."
        info "Try manual: curl -fsSL https://bun.sh/install | bash"
        exit 1
      fi
  ...
  # Fresh install — upgrade to canary to avoid known issues with 1.3.14
  local fresh_ver
  fresh_ver=$(bun --version 2>/dev/null || echo "")
  if [[ "$fresh_ver" == "$BUN_BAD_VERSION" ]]; then
    info "Upgrading fresh install to canary..."
    bun upgrade --canary || warn "Canary upgrade failed (continuing with stable)"
    ok "Bun $(bun --version) (upgraded to canary)"
  fi
}
...
# ── Optional: SearXNG for the built-in web_search tool ─────────────────
# The web_search shim works out-of-the-box with a keyless DuckDuckGo backend.
# SearXNG is an optional, more-robust self-hosted backend. This step is opt-in:
#   - Auto-on when ETTEUM_SEARXNG=1
#   - Otherwise prompts the user (skipped in ETTEUM_YES=1 / unattended mode)
# It uses Docker if available; if Docker is absent it prints manual instructions.
setup_searxng() {
  local want=0
  if [ "${ETTEUM_SEARXNG:-0}" = "1" ]; then want=1; fi

  if [ "$want" = "0" ] && [ "$ASSUME_YES" = "0" ]; then
    printf "\n${C_BOLD}Web search backend${C_RESET}\n"
    printf "  The built-in web_search tool works zero-config (DuckDuckGo).\n"
    printf "  Optionally install ${C_CYAN}SearXNG${C_RESET} (self-hosted, more robust) via Docker?\n"
    printf "  ${C_DIM}[y/N] ${C_RESET}"
    local reply
    read -r reply </dev/tty 2>/dev/null || reply=""
    case "$reply" in y|Y|yes|YES) want=1 ;; esac
  fi

  if [ "$want" = "0" ]; then
    info "SearXNG skipped — web_search will use the DuckDuckGo backend."
    info "You can add SearXNG later: run SearXNG, set SEARXNG_URL in .env, then 'etteum doctor'."
    return 0
  fi

  if ! command -v docker >/dev/null 2>&1; then
    warn "Docker not found — cannot auto-install SearXNG."
    info "Install Docker, then run:"
    info "  docker run -d --name etteum-searxng -p 8080:8080 \\"
    info "    -e SEARXNG_BASE_URL=http://localhost:8080/ \\"
    info "    searxng/searxng:latest"
    info "Then add to $PROJECT_DIR/.env:  SEARXNG_URL=http://localhost:8080"
    info "And enable JSON output: in the container's settings.yml add 'json' under search.formats."
    return 0
  fi

  info "Pulling SearXNG image…"
  docker pull searxng/searxng:latest >/dev/null 2>&1 || { warn "docker pull failed — skipping SearXNG"; return 0; }

  # Reuse an existing container if present (idempotent).
  if docker ps -a --format '{{.Names}}' | grep -qx etteum-searxng; then
    docker start etteum-searxng >/dev/null 2>&1 || true
    info "Existing etteum-searxng container started."
  else
    docker run -d --name etteum-searxng -p 8080:8080 \
      -e SEARXNG_BASE_URL=http://localhost:8080/ \
      searxng/searxng:latest >/dev/null 2>&1 || { warn "docker run failed — skipping SearXNG"; return 0; }
    info "SearXNG container started on http://localhost:8080"
    # Enable JSON output format (default SearXNG disables it).
    # Give the container a moment to create settings.yml, then patch it.
    sleep 2
    docker exec etteum-searxng sh -c "grep -q 'search:' /etc/searxng/settings.yml 2>/dev/null && sed -i '/^search:/,/^[^ ]/{s/^\( *\)formats: \[.*\]$/\1formats: [html, json]/;}' /etc/searxng/settings.yml || true" 2>/dev/null || true
    docker restart etteum-searxng >/dev/null 2>&1 || true
  fi

  # Write SEARXNG_URL into .env if absent.
  if ! grep -q '^SEARXNG_URL=' "$PROJECT_DIR/.env" 2>/dev/null; then
    printf "SEARXNG_URL=http://localhost:8080\n" >> "$PROJECT_DIR/.env"
