...
Bun.serve({
  ...
  async fetch(req) {
    ...
    // Hashed assets (e.g. /assets/Accounts-Abc123.js) can be cached forever.
    // index.html must never be cached so the browser picks up new hashed filenames.
    ...
    if (await file.exists()) {
      return new Response(file, {
        headers: { "Content-Type": getMimeType(pathname), ...cacheHeaders },
      });
    }
